...
public sealed class HostLauncher
{
    ...
    /// <summary>Starts a shell line inside the agent's compositor. Throws when ghostty refuses.</summary>
    public void RunInCompositor(string shellLine)
    {
        if (Has(() => call.HasFunction))
        {
            if (GhosttyWire.ReplyError(call.InvokeFunc(GhosttyWire.OpenRun(shellLine))) is { } error)
